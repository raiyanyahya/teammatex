    ...
    invs = [
        {
            "tool": "semantic_search",
            "args": {},
            "result": _env([{"file_path": "auth.py", "start_line": 1, "end_line": 9}]),
        },
...
def test_errored_tool_calls_produce_no_sources():
    invs = [
        {"tool": "read_file", "args": {"file_path": "x.py"}, "result": {"error": "File not found"}}
    ]
...
async def test_loop_emits_sources_event_after_answer():
    ...
    async def execute_tool(name, args):
        return {
            "success": True,
            "data": [{"file_path": "billing_webhooks.py", "start_line": 1, "end_line": 9}],
        }
