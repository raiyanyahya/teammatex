...
class TestAttachmentInjection:
    async def _make_upload(self, db, owner, filename, data: bytes, tmp_path):
        ...
        p.write_bytes(data)
        up = Upload(owner_id=owner, filename=filename, content_type="text/plain",
                    size_bytes=len(data), stored_path=str(p))
        db.add(up)
        await db.flush()
        return up
    ...
    async def test_prepends_text_file(self, async_db, tmp_path):
        ...
        up = await self._make_upload(async_db, "owner-1", "notes.txt", b"line one\nline two", tmp_path)
    ...
    async def test_binary_file_noted_not_garbled(self, async_db, tmp_path):
        from app.services.agent.attachments import build_attached_message
        up = await self._make_upload(async_db, "owner-1", "img.png", b"\x89PNG\x00\xff\xfe", tmp_path)
