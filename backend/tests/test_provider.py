...
class TestModelName:
    def test_deepseek_v4_flash_passes_through(self):
        assert P.LLMProvider._get_model_name("deepseek", "deepseek-v4-flash") == \
            "deepseek/deepseek-v4-flash"

    def test_deepseek_v4_pro_passes_through(self):
        assert P.LLMProvider._get_model_name("deepseek", "deepseek-v4-pro") == \
            "deepseek/deepseek-v4-pro"

    def test_legacy_deepseek_chat_still_maps(self):
        assert P.LLMProvider._get_model_name("deepseek", "deepseek-chat") == \
...
async def test_chat_returns_without_nameerror(monkeypatch):
    ...
    monkeypatch.setattr(P.LLMProvider, "_get_available_providers",
                        classmethod(fake_providers))
