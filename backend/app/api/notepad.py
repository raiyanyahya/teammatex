...
async def get_notepad(user: dict = Depends(require_user), db: AsyncSession = Depends(get_db)):
    note = (
        await db.execute(select(Notepad).where(Notepad.owner_id == _owner(user)))
    ).scalar_one_or_none()
    return {
...
async def save_notepad(
    ...
    owner = _owner(user)
    note = (await db.execute(select(Notepad).where(Notepad.owner_id == owner))).scalar_one_or_none()
    if note is None:
