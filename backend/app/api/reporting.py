...
async def review_pr(
    ...
    try:
        with Session(engine) as db:
            ...
            if not repo:
                raise HTTPException(status_code=404, detail="Repo not found")
            result = await pr_reviewer.review(
                repo_id, repo.local_name, pr_title, pr_body, changed_files or [], diff
            )
            return result
    finally:
        engine.dispose()
    ...
    try:
        with Session(engine) as db:
            ...
            result = await blame_tracer.trace(
                repo_id, repo.local_name, entity_name, file_path, f"/data/repos/{repo.local_name}"
            )
            return result
    finally:
        engine.dispose()
