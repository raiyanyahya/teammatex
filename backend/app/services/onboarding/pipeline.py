...
def start_onboarding(repo_id: str, github_url: str, local_name: str) -> str:
    ...
    task_chain = chain(
        _stage_task.s(
            ...
            repo_id=repo_id,
            stage=OnboardingStage.REPO_DISCOVERY.value,
        ...
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.HISTORY_MINING.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.CODE_ANALYSIS.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.PEOPLE_PROFILING.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.FEATURE_EXTRACTION.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.GRAPH_BUILDING.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.EMBEDDING.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.SYNTHESIS.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.TECH_DEBT_SCAN.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.STYLE_LEARNING.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.DEPENDENCY_SCAN.value,
            github_url=github_url,
            local_name=local_name,
        ),
        _stage_task.s(
            pipeline_id=pipeline_id,
            repo_id=repo_id,
            stage=OnboardingStage.INTRO_REPORT.value,
            github_url=github_url,
            local_name=local_name,
        ),
    )
...
def _stage_task(
    ...
    try:
        ...
        result = _execute_stage(
            pipeline_id,
            repo_id,
            OnboardingStage(stage) if stage else None,
            prev_result,
            github_url,
            local_name,
        )
...
def _save_stage_status(repo_id: str, stage_name: str, status: str, error: str = None):
    ...
    try:
        ...
        with Session(engine) as db:
            result = db.execute(
                _select(RepoOnboardingState).where(
        ...
        logger.warning(
            "save_stage_status_failed", repo_id=repo_id, stage=stage_name, error=str(e)[:200]
        )
...
def _get_clone_path(prev_result: Any) -> str:
    if isinstance(prev_result, dict):
        ...
        if path:
            return path
    return (
        f"/data/repos/{prev_result.get('name', 'unknown')}" if isinstance(prev_result, dict) else ""
    )
...
def _learn_styles(clone_path: str) -> list[str]:
    ...
    if indent_found:
        ...
        conventions.append(
            f"Indentation: {dominant} ({indent_found[dominant]} of {sum(indent_found.values())} files)"
        )
    return conventions
...
def _build_embeddings_sync(repo_id: str, clone_path: str) -> dict:
    ...
    try:
        ...
        if insert_errors:
            logger.error(
                "embedding_insert_errors",
                repo=repo_id,
                stored=chunks_stored,
                total=len(all_chunks),
                samples=insert_errors,
            )
...
def _persist_memory_sync():
    try:
        ...
        with engine.connect() as conn:
            ...
            conn.execute(
                text("INSERT INTO app_config (key, value) VALUES ('memory_state', :val)"),
                {
                    "val": json.dumps(
                        {"preferences": {}, "conventions": ["Onboarding complete"], "episodic": []}
                    )
                },
            )
            conn.commit()
...
def _create_synthesis_notes_sync(
    repo_id: str, repo_name: str, prev_result: dict, clone_path: str
) -> int:
    import uuid
    from pathlib import Path
    ...
    if key_files:
        notes.append(
            (
                ...
                "## Key Files\n\n"
                + "\n".join(
                    f"- `{str(f.relative_to(root))}` ({s}B)"
                    for f, s in sorted(key_files, key=lambda x: x[1], reverse=True)
                ),
                repo_id,
            )
        )
