...
class DocsGenerator:
    ...
    def _format_architecture(self, modules: list[dict], repo_name: str) -> str:
        ...
        for mod in modules[:20]:
            lines.append(f"| {mod.get('module', 'N/A')} | {mod.get('file_count', 0)} | {mod.get('function_count', 0)} |")
        return "\n".join(lines)
    ...
    def _format_module_graph(self, module_graph: dict, repo_name: str) -> str:
        ...
        if edges:
            ...
            for edge in edges[:15]:
                lines.append(f"- `{edge.get('source')}` → `{edge.get('target')}` (weight: {edge.get('weight', 0)})")
